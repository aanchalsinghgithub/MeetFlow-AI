...
// BUGFIX: MeetingSummary already existed (built to show summary/discussion
// points/decisions/risks/blockers for a meeting) but nothing ever imported
// it, so the Approval Queue had no way to show "what was discussed" for the
// meeting a batch of tasks came from - only the bare folder title. Reusing
// it here instead of duplicating its rendering logic.
...
export default function ApprovalQueue() {
  ...
  // NEW: which meeting groups currently have their summary panel open.
  // Collapsed by default so the queue still reads like a task list first.
          ...
          <div key={group.meetingId ?? 'none'} style={styles.meetingGroup}>
            <div style={styles.meetingHeader}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                {group.meetingId != null && (
                  <button
                    className="ghost"
                    style={styles.summaryToggle}
                    onClick={() => toggleSummary(group.meetingId)}
                  >
                    {expandedSummaries.has(group.meetingId) ? '▲ Hide summary' : '▾ View summary'}
                  </button>
                )}
              </div>
            </div>

            {group.meetingId != null && expandedSummaries.has(group.meetingId) && (
              <div style={styles.summaryPanel}>
                <MeetingSummary meeting={{ id: group.meetingId, status: group.meetingStatus }} />
              </div>
            )}
