...
class MeetingPipeline:
    ...
    async def process_transcript(self, payload: ProcessTranscriptRequest) -> ProcessTranscriptResponse:
        ...
        for task_payload in task_payloads:
            ...
            task = self.tasks.create(task_payload, self.organization_id)
            self.approvals.create_manager_review(task.id, self.organization_id)
    ...
    def _get_or_create_meeting(self, payload: ProcessTranscriptRequest) -> Meeting:
        if payload.meeting_id:
            meeting = (
                ...
                .filter(
                    ...
                    Meeting.organization_id == self.organization_id,
                )
                .one_or_none()
            )
            if meeting:
        ...
        meeting = Meeting(title=payload.meeting_title, organization_id=self.organization_id)
        self.db.add(meeting)
        self.db.flush()
