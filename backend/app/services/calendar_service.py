...
# In-memory store: {state -> code_verifier} for PKCE between connect and callback
...
class CalendarIntegrationService:
    ...
    def get_authorization_url(self) -> str:
        ...
        # Store verifier so the callback can retrieve it by state
    ...
    def exchange_code(self, code: str, state: str, db: Session) -> CalendarConnection:
        ...
        connection = (
            ...
            .filter(CalendarConnection.user_email == user_email)
            .one_or_none()
        )
    ...
    def _upsert_meeting(self, db: Session, connection: CalendarConnection, event: dict, join_url: str) -> Meeting:
        ...
        meeting = (
            db.query(Meeting)
            .filter(Meeting.external_id == external_id, Meeting.provider == MeetingProvider.GOOGLE_MEET.value)
            .one_or_none()
        )
