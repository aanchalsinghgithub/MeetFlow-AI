...
class ApprovalService:
    ...
    def create_manager_review(self, task_id: int, organization_id: int) -> None:
        task = self.tasks.get(task_id, organization_id)
        if not task:
            return
        manager_email = leader_for_domain(task.domain) or "manager@company.com"
        self.tasks.add_approval(task.id, manager_email, organization_id)
        self.email.approval_request(manager_email, task)

    def decide(self, task_id: int, payload: ApprovalUpdate, organization_id: int) -> dict:
        ...
        approval = self.tasks.latest_approval(task.id, organization_id)
        if approval:
