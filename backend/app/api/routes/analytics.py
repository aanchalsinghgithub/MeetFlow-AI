...
def summary(
    ...
    meetings = db.query(func.count(Meeting.id)).filter(Meeting.organization_id == org_id).scalar() or 0
    tasks = db.query(func.count(Task.id)).filter(Task.organization_id == org_id).scalar() or 0
    approved = (
        db.query(func.count(Task.id))
        .filter(Task.organization_id == org_id, Task.status.in_(["approved", "assigned"]))
        .scalar()
        or 0
    )
    pending = (
        db.query(func.count(Task.id))
        .filter(Task.organization_id == org_id, Task.status == "review_required")
        .scalar()
        or 0
    )
    avg_confidence = (
        db.query(func.avg(Task.confidence)).filter(Task.organization_id == org_id).scalar() or 0
    )
    by_domain = (
        db.query(Task.domain, func.count(Task.id))
