...
@router.get("/queue")
def approval_queue(db: Session = Depends(get_db)) -> list[dict]:
...
def decide(task_id: int, payload: ApprovalUpdate, db: Session = Depends(get_db)) -> dict:
    ...
    try:
        email_status = ApprovalService(db).decide(task_id, payload)
