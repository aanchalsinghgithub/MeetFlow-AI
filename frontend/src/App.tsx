...
export function App() {
  ...
  const [authPage, setAuthPage] = useState<'welcome' | 'login' | 'signup'>('welcome');
  ...
  if (!auth.isAuthenticated) {
    if (authPage === 'welcome') {
      return <Welcome onLogin={() => setAuthPage('login')} onSignup={() => setAuthPage('signup')} />;
    }
    return authPage === 'login'
      ? <Login onShowSignup={() => setAuthPage('signup')} onBack={() => setAuthPage('welcome')} />
      : <Signup onShowLogin={() => setAuthPage('login')} onBack={() => setAuthPage('welcome')} />;
  }
