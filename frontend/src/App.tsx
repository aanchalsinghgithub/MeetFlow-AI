...
import { BotAccountSettings } from "./components/BotAccountSettings";
...
const sampleTranscript = {
  ...
  transcript: [
    ...
    {
      speaker: "Client",
      text: "We need dashboard filters fixed before next week's demo.",
    },
  ],
};
...
export function App() {
  ...
  const [authPage, setAuthPage] = useState<"welcome" | "login" | "signup">(
    "welcome",
  );
  ...
  const analytics = useQuery({
    queryKey: ["analytics"],
    queryFn: async () =>
      (await api.get<AnalyticsSummary>("/api/analytics/summary")).data,
    initialData: {
  ...
  const upcomingMeetings = useQuery({
    ...
    queryFn: async () =>
      (await api.get<UpcomingMeeting[]>("/api/meetings/upcoming")).data,
    initialData: [],
    ...
    <div className="min-h-screen bg-[#f7f8f5] text-ink">
      ...
      <main className="md:pl-64">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-stone-500">
                Autonomous meeting intelligence and approval routing
              </p>
            </div>
            <button
            ...
            <button
              className="inline-flex h-10 items-center rounded border border-stone-300 px-4 text-sm font-medium hover:bg-stone-50"
              onClick={() => {
                auth.logout();
                queryClient.clear();
              }}
        ...
        <section className="p-5">
          ...
          {activePage === "Meetings" && (
            <MeetingsDashboard onOpenTranscript={openTranscript} />
          )}
          ...
          {activePage === "Team Analytics" && (
            <TeamAnalytics summary={analytics.data} />
          )}
          ...
          <div className="flex items-center gap-2 rounded border border-stone-200 bg-white p-3">
            ...
            <select
              ...
              onChange={(event) => {
                ...
                onSelectMeeting(
                  meeting ? { id: meeting.id, title: meeting.title } : null,
                );
              }}
            >
    ...
    <div className="rounded border border-stone-200 bg-white p-4">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {["Approve", "Edit", "Reject"].map((action) => (
          <button
            key={action}
            className="h-10 rounded border border-stone-300 bg-white text-sm font-medium hover:bg-stone-50"
          >
            {action}
          </button>
        ))}
...
function TeamAnalytics({ summary }: { summary: AnalyticsSummary }) {
  return (
    <Chart
      title="Team Workload and Domain Routing"
      data={summary.tasks_by_domain}
    />
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {["Mistral API", "SMTP Email", "Slack Webhook", "Calendar OAuth"].map(
          (item) => (
            <div
              key={item}
              className="rounded border border-stone-200 bg-white p-4"
            >
              ...
              <p className="text-sm text-stone-600">
                Configure through environment variables and organization
                secrets.
              </p>
            </div>
          ),
